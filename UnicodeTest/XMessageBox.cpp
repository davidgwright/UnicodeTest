...
// These may not work - may need to add our own resources?
#define IDI_HAND            MAKEINTRESOURCE(201)
#define IDI_QUESTION        MAKEINTRESOURCE(202)
#define IDI_EXCLAMATION     MAKEINTRESOURCE(200)
#define IDI_ASTERISK        MAKEINTRESOURCE(203)
...
LONG CALLBACK IconProc(HWND hwnd, UINT message, WPARAM, LPARAM)
{
	if (message == WM_PAINT)
	{
		...
		::DrawIconEx(hdc, 0, 0, (HICON)(::GetWindowLong(hwnd, GWLP_USERDATA)), 0, 0, 0, 0, DI_NORMAL );
		EndPaint(hwnd, &ps);
	}
...
*/
   ...
   _tcscpy(logFont.lfFaceName, TEXT("Arial Unicode MS"));
   ...
   //_ASSERTE(ok);
	...
	else if (nStyle & MB_ICONMASK)
	{
		...
		if (lpIcon)
			m_hIcon = ::LoadIcon(hInstanceIcon, lpIcon);
	}
...
INT_PTR CALLBACK CXDialogTemplate::MsgBoxDlgProc(HWND hwnd,
												 ...
												 LPARAM lParam)
{
	CXDialogTemplate* Me = (CXDialogTemplate*) ::GetWindowLong(hwnd, GWLP_USERDATA);
	HWND hwndChild = 0;
	...
	switch (message)
	{
		...
		case WM_INITDIALOG:
		{
			...
			::SetWindowLong(hwnd, GWLP_USERDATA, lParam);	// save it for the others
			...
			if (Me->m_hIcon)
			{
				...
				::SetWindowLong(hwndIcon, GWLP_WNDPROC, (LONG_PTR) IconProc);
				::SetWindowLong(hwndIcon, GWLP_USERDATA, (LONG_PTR) Me->m_hIcon);
			}
			...
			switch (wParam)
			{
				...
				case IdExHelp:
				{
					...
					if (_tcslen(szBuf) > 0)
					{
						...
						{
							// DGW - Fix Later?
							::MessageBox( NULL, L"Help Not Supported", L"Error", MB_OK);
							/* 
							_tcscpy(cp, _T(".hlp"));
							::WinHelp(hwnd, szBuf,
	...
	*(WORD*)pdest = 0;		// use default window class		//+++1.5
	...
	if (hDlg)
	{
		...
		while (!IsEnded())
		{
			...
			{
				// DGW - let's just see what happens!?
